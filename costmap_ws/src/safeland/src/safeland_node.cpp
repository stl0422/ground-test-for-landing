  ...
  void gridMapCallback(const grid_map_msgs::GridMap::ConstPtr& msg) {
    ...
    try {
      ...
      ROS_INFO_THROTTLE(5.0,
          "[safeland] #%lu | %dx%d, %d valid | count=%s min_pts=%.1f | "
          "smooth_k=%d step_k=%d land_k=%d eval_k=%d dep_bg_k=%d dep_score_k=%d | %.1f ms",
          frame_seq_, rows, cols, valid_count, has_count_layer ? "on" : "off", min_cell_points_,
